...
class trimImage:
    ...
    def trimming(self, listOfTsv:list, outputFolder:str):
        for l in listOfTsv:
            top = l[Label.top]
            ...
            imageName = str(l[Label.page_num]) + "_" + str(l[Label.block_num]) + "_" + str(l[Label.par_num]) + "_" + str(l[Label.line_num])
            imagePath = os.path.join(outputFolder,imageName + ".jpg")
            print("making image  " + imagePath + "  ...", end = "")
            cv2.imwrite(imagePath, trimmedImage)
            print("success!")
